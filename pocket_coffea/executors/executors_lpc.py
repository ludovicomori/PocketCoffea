...
def _resolve_host_path(path):
    """Translate /srv/... paths back to real host paths when running inside Apptainer.

    The shell launcher does `apptainer exec -B $HOST_PWD:/srv` and sets HOST_PWD
    in the environment. os.path.abspath() inside the container returns /srv/...,
    which is invalid on condor execute nodes that don't have the container mounted.
    """
    if path.startswith("/srv"):
        host_pwd = os.environ.get("HOST_PWD")
        if host_pwd:
            return host_pwd + path[len("/srv"):]
    return path
...
class ExecutorFactoryCondorLPC(ExecutorFactoryManualABC):
    ...
    def submit_jobs(self, jobs_config):
        ...
        abs_output_path = _resolve_host_path(os.path.abspath(self.outputdir))
        abs_jobdir_path = _resolve_host_path(os.path.abspath(self.jobs_dir))
...
"""
    ...
    def recreate_jobs(self, jobs_to_recreate):
        ...
        host_jobs_dir = _resolve_host_path(os.path.abspath(self.jobs_dir))
